...
# 构造合并转发消息（适配 Hoshino/NoneBot）
async def make_forward_msg(bot, ctx, msg_list: list, title: str = "问答列表", brief: str = "查看问答内容"):
    """
    构造合并转发消息
    :param bot: 机器人实例
    :param ctx: 消息上下文
    :param msg_list: 消息列表 [{"name": 发送者名, "uin": 发送者QQ, "content": 消息内容}, ...]
    :param title: 转发卡片标题
    :param brief: 转发卡片简介
    :return: 合并转发消息
    """
    # 适配 Hoshino 的合并转发（如果用原生 NoneBot 需调整）
    forward_msg = []
    for msg in msg_list:
        forward_msg.append({
            "type": "node",
